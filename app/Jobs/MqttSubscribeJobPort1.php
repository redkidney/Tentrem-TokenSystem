...
class MqttSubscribeJobPort1 implements ShouldQueue
{
    ...
    {
        try {
            Log::info('(MqttSubscribeJobPort1) MQTT Subscription started.');

            // Connect to MQTT service
            $mqttService->connect();

            // Subscribe to port 1's topic
            $this->subscribeToPort($mqttService, 1);

            // Keep the MQTT connection alive and listening
            $mqttService->loop();
        } catch (\Exception $e) {
            Log::error('(MqttSubscribeJobPort1) Failed to subscribe to MQTT topics: ' . $e->getMessage());
        }
    }
    ...
    {
        Log::info("(MqttSubscribeJobPort1) Subscribing to port $port");
    ...
    {
        Log::info("(MqttSubscribeJobPort1) Received MQTT message from port {$port}: " . $message);
    ...
    {
        try {
            Log::info("(MqttSubscribeJobPort1) Starting charging process for port {$port}");
            
            // Dispatch the ChargingStatus event
            event(new ChargingStatus('charging_started', $port));
            
        } catch (\Exception $e) {
            Log::error("(MqttSubscribeJobPort1) Error starting charging for port {$port}: " . $e->getMessage());
        }
    }
}